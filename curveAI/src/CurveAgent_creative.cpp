...
void CurveAgent::reset(const SteerLib::AgentInitialConditions & initialConditions, SteerLib::EngineInterface * engineInfo)
{
	...
	_radius = 2+initialConditions.radius;
	_velocity = 2*initialConditions.speed * Util::normalize(initialConditions.direction);//1*
	...
	else
	{
		srand(time(NULL));
		float r, g, b;
		r = (float)(rand() % 50) / 100.0f;//80
		g = (float)(rand() % 90) / 100.0f;//80
		b = (float)(rand() % 30) / 100.0f;//80
